...
| **A slash command** | Three synchronized spots in `harness-cli`: a `Command` variant + its parse arm in [`repl.rs`](crates/harness-cli/src/repl.rs), a dispatch arm in [`repl_loop.rs`](crates/harness-cli/src/repl_loop.rs) calling your new `*_cmd.rs` module, and a `SLASH_COMMANDS` entry in [`live/mod.rs`](crates/harness-cli/src/live/mod.rs) (a test fails if the three drift). Model a command with subcommands on [`loop_cmd.rs`](crates/harness-cli/src/loop_cmd.rs). |
